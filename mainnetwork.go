...
func (net *Network) Train(inputData []float64, targetData []float64) {
	...
	fmt.Println("Traing Targets: ", targets)
...
// predict a number from an image
// image should be 28 x 28 PNG file
func predictFromMemoryImage(net Network, img image.Image) int {
	input := prepareImage(img)
	output := net.Predict(input)
	matrixPrint(output)
	best := 0
	highest := 0.0
...
// get the pixel data from an image
func prepareImage(img image.Image) (pixels []float64) {
